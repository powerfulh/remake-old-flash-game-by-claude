...
- **FILL 규칙**: DIG로 흙을 퍼 와야 FILL 가능으로 구현 (`CONFIG.fillRequiresDirt`).
  원본 규칙 불확실 — 플레이 검증 필요.
- **전투 모델**: HP 100 고정, 피해 = damage_range 균등난수 × 대상 shield,
  공격 주기 = 60/hits_per_minute 초, 명중률 chance%. 원본 수치 해석 — 밸런스 검증 필요.
- **파괴 시**: 유닛/몬스터 모두 레시피 브릭을 그 자리에 드랍 (원본의 "분해" 동작).
- **조립**: 대상 칸 + 8방향 칸의 브릭 더미에서 레시피 소모. 유닛 근접 불필요(원본 추정).
